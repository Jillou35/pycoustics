...
async def audio_websocket(websocket: WebSocket, session_id: str | None = None, db: Session = Depends(get_db)):
    ...
    try:
        while True:
            ...
            if message["type"] == "websocket.receive":
                if "bytes" in message and message["bytes"]:
                    ...
                    # 1. Process
                    processed_chunk, rms_db, spectrum, panning = processor.process_chunk(audio_chunk)
                    ...
                    await websocket.send_json({"type": "meter", "rms": rms_db, "spectrum": spectrum, "panning": panning})
                    ...
                    try:
                        ...
                        if action == "init":
                            ...
                            logger.info(f"Initialized AudioProcessor with sample_rate={sample_rate}, channels={channels}")

                        elif action == "start_record":
                            # Update params if provided, otherwise use current
                            sample_rate = int(data.get("sample_rate", processor.sample_rate))
                            channels = int(data.get("channels", processor.channels))
                            
                            recorder.start_recording(settings={
                                "gain": processor.gain,
                            ...
                            logger.info(f"Started recording for session {session_id} at {sample_rate}Hz, {channels}ch")
                        
                        elif action == "stop_record":
                            entry = await recorder.stop_recording(db)
                            if entry:
                                 await websocket.send_json({"type": "recording_saved", "id": entry.id, "filename": entry.filename})
                                 logger.info(f"Stopped recording. Saved to {entry.filename}")
        ...
        if session_id:
            ...
            recordings = db.query(RecordingModel).filter(RecordingModel.session_id == session_id).all()
...
def get_recordings(session_id: str | None = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)) -> List[Recording]:
    ...
    recordings = query.order_by(RecordingModel.timestamp.desc()).offset(skip).limit(limit).all()
    return recordings
...
def delete_recording(filename: str, db: Session = Depends(get_db)) -> dict:
    ...
    """
    # 1. Delete from DB
    recording = db.query(RecordingModel).filter(RecordingModel.filename == filename).first()
    if not recording:

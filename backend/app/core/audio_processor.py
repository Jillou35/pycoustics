...
class AudioProcessor:
    ...
    def update_settings(self, gain: float = 0.0, filter_enabled: bool = False, cutoff_freq: float | None = None, integration_time: float = 0.5):
        self.gain = gain
    ...
    def process_chunk(self, chunk_bytes: bytes) -> Tuple[bytes, float, list[float], float]:
        """
    ...
    def _apply_dsp_effects(self, audio_stereo: np.ndarray) -> np.ndarray:
        ...
        if self.filter_enabled:
            ...
            audio_stereo, self.zi = signal.lfilter(self.b, self.a, audio_stereo, axis=0, zi=self.zi)
            
        return audio_stereo

    def _analyze_audio(self, audio_stereo: np.ndarray) -> Tuple[float, list[float], float]:
    ...
    def _apply_smoothing(self, rms_db: float, spectrum: list[float], panning: float) -> Tuple[float, list[float], float]:
        ...
        if self._rms_db_smooth < -99.0:
            ...
            self._rms_db_smooth = (alpha * self._rms_db_smooth) + ((1.0 - alpha) * rms_db)
            
        # Smooth Panning
        self._panning_smooth = (alpha * self._panning_smooth) + ((1.0 - alpha) * panning)
        ...
        return float(self._rms_db_smooth), self._spectrum_smooth, float(self._panning_smooth)
